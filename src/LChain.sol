...
contract LChainHook is BaseHook, ERC1155 {
    ...
    // Create a nested mapping to store the l-chain orders placed by users
    // The mapping is PoolId => tickLower => zeroForOne => expression
    ...
    // expressiono specifies the condition to sell and if the user wants to buy into a different LP contract
    mapping(PoolId poolId => mapping(int24 tick => mapping(bool zeroForOne => string expression)))
        public lchainOrders;
    ...
    function afterInitialize(
        ...
        return LChainHook.afterInitialize.selector;
    }

    // after a swap occurs, the hook updates the price
    function afterSwap(
        address,
        PoolKey calldata key,
        ...
        if (lastTickLower < currentTickLower) {
            ...
            for (int24 tick = lastTickLower; tick < currentTickLower; ) {
                expression = lchainOrders[key.toId()][tick][
                    swapZeroForOne
                ];
                if (expression != "") {
                    fillOrder(key, tick, swapZeroForOne, expression);
                }
        ...
        return LChainHook.afterSwap.selector;
    }
    ...
    function placeOrder(
        ...
        lchainOrders[key.toId()][tickLower][zeroForOne] += int256(
            amountIn
        );
    ...
    function fillOrder(
        ...
        int24 tick,
        bool zeroForOne,
        string expression // empty string checked before
    ) internal {
        // 
        ...
        lchainOrders[key.toId()][tick][zeroForOne] -= amountIn;
